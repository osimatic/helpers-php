...
class WebPushNotificationOldSender
{
	...
	{
		$opt = [
			'renotify' => $this->webPushNotificationPayload->isReNotify(),
			'requireInteraction' => $this->webPushNotificationPayload->isRequireInteraction(),
			'silent' => $this->webPushNotificationPayload->isSilent(),
			'data' => []
		];

		if (!empty($this->webPushNotificationPayload->getTag())) {
			$opt['tag'] = $this->webPushNotificationPayload->getTag();
		}
		if (!empty($this->webPushNotificationPayload->getImage())) {
			$opt['image'] = $this->webPushNotificationPayload->getImage();
		}
		if (!empty($this->webPushNotificationPayload->getBadge())) {
			$opt['badge'] = $this->webPushNotificationPayload->getBadge();
		}

		if (null !== ($dateTime = $this->webPushNotificationPayload->getDateTime())) {
			$opt['timestamp'] = $dateTime->getTimestamp() * 1000;
		}

		if (!empty($this->webPushNotificationPayload->getUrl())) {
			$opt['data']['url'] = $this->webPushNotificationPayload->getUrl();
		}

		if (!empty($this->webPushNotificationPayload->getVibrate())) {
			$opt['vibrate'] = $this->webPushNotificationPayload->getVibrate();
		}
		if (!empty($this->webPushNotificationPayload->getSound())) {
			$opt['sound'] = $this->webPushNotificationPayload->getSound();
		}

		if (!empty($this->webPushNotificationPayload->getActions())) {
			$opt['actions'] = [];
			foreach ($this->webPushNotificationPayload->getActions() as $action) {
				$opt['actions'][] = [
